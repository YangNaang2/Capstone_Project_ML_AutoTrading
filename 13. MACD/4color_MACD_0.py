...
class FourColorMACDStrategy(bt.Strategy):
    def __init__(self):
        # MACD 구성 요소 계산
    ...
    def next(self):
        ...
        # 4색 MACD 로직 적용
        is_lime = curr_macd > 0 and curr_macd > prev_macd
        is_green = curr_macd > 0 and curr_macd < prev_macd
        is_maroon = curr_macd < 0 and curr_macd < prev_macd
        is_red = curr_macd < 0 and curr_macd > prev_macd

        # 매수: lime 조건일 때만 진입
        if is_lime and not self.position:
            self.buy()
            self.log(f"Buy → MACD: {curr_macd:.4f}, Prev: {prev_macd:.4f}")

        # 청산: maroon 조건일 때 포지션 청산
        elif is_maroon and self.position:
            self.close()
            self.log(f"Sell → MACD: {curr_macd:.4f}, Prev: {prev_macd:.4f}")
...
cerebro.addstrategy(FourColorMACDStrategy)
