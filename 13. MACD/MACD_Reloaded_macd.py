...
class MACDReloadedMACDMode(bt.Strategy):
    params = (
        ('fast', 12),
        ('slow', 26),
        ('signal', 9),
    ...
    def next(self):
        ...
        if prev_macd < 0 and curr_macd > 0 and not self.position:
            self.buy()
            self.log(f"BUY: MACD crossed above 0 ->{curr_macd:.4f}")
        elif prev_macd > 0 and curr_macd < 0 and self.position:
            self.close()
            self.log(f"SELL: MACD crossed below 0 ->{curr_macd:.4f}")
    ...
    def stop(self):
        ...
        pd.DataFrame(self.logs).to_csv("./log_dir/macd_reloaded_macd.csv", index=False)


cerebro.addstrategy(MACDReloadedMACDMode)
# PyFolio 분석기 추가
...
report_filename = f"{results_dir}/MACD_Reloaded_macd_{int(time.time())}.html"
qs.reports.html(returns, output=report_filename,
                download_filename=report_filename,
                title="MACD_Reloaded_macd")
