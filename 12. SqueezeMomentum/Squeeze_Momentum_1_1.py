...
class SqueezeMomentumStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        order_size = self.broker.getcash() * self.p.risk_ratio / 100 / price

        # 매수 시그널: 음 -> 양
        if prev_hist < 0 and curr_hist > 0:
            if self.position.size < 0:
                self.close()
                self.log(f"[숏 청산] 가격: {price:.2f}, 수량: {abs(self.position.size):.10f}, 모멘텀: {curr_hist:.6f}")
            if not self.position or self.position.size <= 0:
                if abs(curr_hist) < 1000:
                    self.buy(size=order_size)
                    self.log(f"[롱 진입] 가격: {price:.2f}, 수량: {order_size:.10f}, 모멘텀: {curr_hist:.6f}")

        # 매도 시그널: 양 -> 음
        elif prev_hist > 0 and curr_hist < 0:
            if self.position.size > 0:
                self.close()
                self.log(f"[롱 청산] 가격: {price:.2f}, 수량: {self.position.size:.10f}, 모멘텀: {curr_hist:.6f}")
            if not self.position or self.position.size >= 0:
                if abs(curr_hist) < 1000:
                    self.sell(size=order_size)
                    self.log(f"[숏 진입] 가격: {price:.2f}, 수량: {order_size:.10f}, 모멘텀: {curr_hist:.6f}")
